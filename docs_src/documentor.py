...
class EmptyStringArray(object):
    ...
    This class works as a indexable container which always returns
    an empty string.
    '''
...
def save_table_imgs(schema, main_diag, img_dir):
    '''Save table diagrams as PlantUML scritps in txt files.'''
    for table_name, table in schema['tables'].iteritems():
        diag = main_diag.focus_on(table_name)
...
def main():
    ...
    arg_parser.add_argument('--output-dir', type=str,
                            help='directory to save MD documents')
    args = arg_parser.parse_args()
    if not args.output_dir.endswith('/'):
        args.output_dir = args.output_dir + '/'

    # Read schema
    with open(args.unified_schema_file, 'r') as fp:
    ...
    # Save to files
    save_tables(doc, args.output_dir)
    save_table_imgs(schema, diag, args.output_dir + 'img/')
